#!/usr/bin/env python3
"""
train_cnn_dense_metrics.py

Dense CNN training (Fashion-MNIST / CIFAR-10) with M1/MPS-friendly metrics.
...
class DenseCNN(nn.Module):
    ...
    Conv -> BN -> ReLU -> Pool
    Conv -> BN -> ReLU -> Pool
    Dropout -> FC(128) -> Dropout -> FC(num_classes)
    """
    def __init__(self, in_ch: int, img_size: int, num_classes: int, dropout: float):
        ...
        # compute feature dim deterministically
        with torch.no_grad():
...
def main():
    ...
    p.add_argument("--run-name", type=str, default="cnn_dense_fullmetrics")
    args = p.parse_args()
    ...
    model = DenseCNN(in_ch=in_ch, img_size=img_size, num_classes=num_classes, dropout=args.dropout).to(device)
